...
class FolderField(FolderDataElement):
    ...
    def Get3DFieldFrom2DSliceInOriginalUnits(self, timeStep, transvEl, longEl):
        ...
        Rout = np.sqrt(X**2 + Y**2)
        field3D[int(transvEl/2):transvEl+1,int(transvEl/2):transvEl+1] = ip.griddata(np.column_stack((Rin,Zin)), field2D, (Rout, Z)) # top right section when looking back from z to the x-y plane.
